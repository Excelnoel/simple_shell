...
int ChangeDirectory(info_t *info)
{
	...
	if (!info->argv[1])
	{
		dir = Getenvar(info, "HOME=");
		if (!dir)
			chdir_ret = /* TODO: what should this be? */
				chdir((dir = Getenvar(info, "PWD=")) ? dir : "/");
		else
			chdir_ret = chdir(dir);
	}
	else if (_strcmp(info->argv[1], "-") == 0)
	{
		if (!Getenvar(info, "OLDPWD="))
		{
			_puts(s);
			_putchar('\n');
			return (1);
		}
		_puts(Getenvar(info, "OLDPWD=")), _putchar('\n');
		chdir_ret = /* TODO: what should this be? */
			chdir((dir = Getenvar(info, "OLDPWD=")) ? dir : "/");
	}
	else
	...
	{
		_setenv(info, "OLDPWD", Getenvar(info, "PWD="));
